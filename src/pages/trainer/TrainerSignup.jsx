...
import { trainerSignup } from "../../api/trainerApi";
...
export default function TrainerSignup() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const res = await trainerSignup(data);
  ...
  return (
    <AuthCard title="Create trainer account" subtitle="We will email you an OTP to verify">
      <Message type="error" text={error} />

      <p className="mb-4 rounded-lg bg-emerald-50 px-4 py-3 text-xs text-emerald-800">
        Signing up also creates your payment (Stripe) account automatically in the
        background, so you can get paid for your courses.
      </p>
      ...
      <p className="mt-4 text-center text-sm text-slate-500">
        ...
        <Link to="/trainer/login" className="text-emerald-600 hover:underline">Login</Link>
      </p>
    </AuthCard>
  );
}
